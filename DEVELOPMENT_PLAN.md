...
- 自研 `game.py` 对局状态机：
  ...
  - 胜负判定；输出对局记录
- **出牌倒计时**（防卡局核心）：每手默认 **30 秒**（市面主流约 15~30 秒、可配置 `PLAY_TIMEOUT`）。
  - 超时且是**领出**（必须先出大省不得过）→ 自动出**最小合法牌**（规则 AI 首出策略）
  - 超时且可不出 → 自动判定**过**
  - 适用对象包括真人、AI（LLM 卡死/超时同样被自动驾驶兜住，保证对局永远能推进）
  - 倒计时从"轮到该玩家"开始算；配 SocketIO 定时广播剩余秒数给前端渲染
...
做：
- `key_picker.py`：**每调用一次轮换下一个 key（7 个均匀轮询）**，不把一个账号用完再换下一个——配合 SenseNova 5 小时滚动窗口；某 key 触发 429/余额受限时临时跳过，窗口期后自动恢复参与轮询；轮询日志留痕
...
### 房间大厅与局域网发现（不用房号也能找到房间）
> 因为"本机作为服务器"，所有局域网设备访问的是**同一个后端**，所以「扫描所有房间」用**房间列表接口**实现即可——浏览器不能收 UDP 组播，多服务器场景才需要真组播，本项目不做。

- 房间状态机：`waiting`（等待中）→ `playing`（游戏中）→ `finished`（结算完可继续/解散）
- **就绪机制**：所有真人座位必须**准备**，全部就绪 + 房主才能点**开始**；AI 座位自动视为就绪
- `GET /api/rooms`：返回实时房间列表（房号 / 人数 / 房间状态 / 底分与座位难度），前台页面轮询刷新（约 2s）
- **大厅默认展示房间列表**（相当于"扫描局域网所有房间"），人少时直接点进；**房号搜索**用于房间多时的精确加入
- 开房可配：底分场（不同门槛+首赌豆）、AI 难度、席位（1~3 真人，空位 AI 补位）

做：
- `rooms.py`：6 位房号、创建/加入/退出、座位抢占（真人入座自动顶替 AI）、房间状态机
- `GET /api/rooms` 列表 + SocketIO 事件广播房间人数/状态变更
- SocketIO 事件：`join/create/deal/bid/play/pass/state/reconnect/leave/chat/comment/ready/start`（`chat`=真人聊天，`comment`=AI 评论）
- 广播策略：整房状态按需全量下发 + 增量事件
- 空位 AI 自动补位（难度默认 `basic`，各座位可独立配置 `basic`/`douzero`/`llm`）
- 断线保留座位 30s，重连后全量同步（SocketIO 自带 reconnect 事件做触发）
- 接入阶段 2.5 结算：`game_over` → `beans.py` 结算入库 → 广播结算面板
- `scripts/ws_clients.py`：3 个模拟客户端加入同一房间完整打完一局

验收标准：
- [ ] 3 个 SocketIO 客户端同一房间完整打完一局（含结算落库）
- [ ] `GET /api/rooms` 正确反映：人数、状态（等待中/游戏中/结算完）
- [ ] 未全就绪时房主点开始被拒；全就绪后才可开始
- [ ] 1 真人 + 2 AI 场景广播正确；断线重连续传正常
- [ ] 多房间隔离；房号搜索与列表加入两条路径都能进房

我的测试：
```
pytest backend/tests/test_ws_game.py -v
pytest backend/tests/test_rooms.py -v          # 就绪机制 / 状态机 / 列表
python backend/scripts/ws_clients.py 123456
curl http://127.0.0.1:8000/api/rooms           # 应在列表看到 123456 waiting
```
...
做：
- **大厅（房间列表）页**：`/api/rooms` 轮询刷新（约 2s），展示所有房间（房号/人数/状态/底分），点房间直接加入；房号搜索框精确加入；可创建房间（选底分场 + AI 难度 + 席位）
- 房间页：座位显示（真人/AI 标签、难度、人格）、准备按钮、就绪状态灯、房主开始按钮（全就绪才可点）
- 游戏页：手牌渲染（CSS/SVG 画牌）、出牌区、上下家剩牌、回合提示、**出牌倒计时**（剩余秒数环形/数字）、叫地主、出牌动画
- 结算面板：欢乐豆增减明细（底分/炸弹翻倍/春天逐项展示）
- AI 评论展示：聊天气泡 + 弹幕样式（`comment` 事件），真人聊天区（`chat`），两者样式区分
- `socket.io-client` 对接后端，状态全部服务端广播驱动
- 单机模式 = 1 真人 + 2 AI 本机房间（自动就绪）

验收标准：
- [ ] `npm run dev` 启动无报错
- [ ] 大厅能看到局域网内的房间列表并直接加入；房号搜索也能进
- [ ] 一个浏览器窗口可对 2 AI 完整打一局（含结算面板）
- [ ] 同局域网两台机器 + 房间列表 可联机对战
...
- [x] 阶段 1：复用牌型引擎 + 游戏状态机
- [x] 阶段 2：规则 AI 兜底
- [x] 阶段 2.5：账号系统 + 欢乐豆（SQLite）
- [x] 阶段 3：LLM 决策层
- [x] 阶段 4：DouZero 强 AI
- [x] 阶段 4.5：AI 评论系统
- [x] 阶段 5：SocketIO 联机房间
- [x] 阶段 6：前端界面
- [x] 阶段 7：体验完善与一键启动